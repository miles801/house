...
//            }
    ...
    private void setCriteriaCondition(Criteria criteria, String fieldName, FilterFieldType filterFieldType, boolean hasPosition, boolean hasOrg, boolean hasParam, boolean hasOrgAndChildren, Set<String> otherOrgIds, Set<String> otherParams) {
        ...
        if (FilterFieldType.EMPLOYEE.equals(filterFieldType)) {
            disjunction.add(Property.forName(fieldName).eq(SecurityContext.getEmpId()));
            // 其他条件
        ...
        if (FilterFieldType.ORG.equals(filterFieldType)) {
            if (hasOrgAndChildren) {    // 查询组织机构及子组织机构
                disjunction.add(Property.forName(fieldName).in(positionEmpDao.findCurrentAndChildOrgIds(SecurityContext.getEmpId())));
                hasOrgAndChildren = false;
            } else if (hasOrg) {
                disjunction.add(Property.forName(fieldName).in(positionEmpDao.findEmpOrgIds(SecurityContext.getEmpId())));
                hasOrg = false;
            }
        ...
        if (FilterFieldType.POSITION.equals(filterFieldType)) {
            if (hasPosition) {
                disjunction.add(Property.forName(fieldName).in(positionEmpDao.findEmpPositionIds(SecurityContext.getEmpId())));
                hasPosition = false;
            }
        ...
        if (FilterFieldType.PARAM.equals(filterFieldType)) {
            if (hasParam) {
                disjunction.add(Property.forName(fieldName).in(positionEmpDao.findEmpParams(SecurityContext.getEmpId())));
                hasParam = false;
            }
    ...
    private Disjunction getDisjunction(boolean hasPosition, boolean hasOrg, boolean hasParam, boolean hasOrgAndChildren, Set<String> otherOrgIds, Set<String> otherParams) {
        ...
        if (hasOrgAndChildren) {    // 查询组织机构及子组织机构
            disjunction.add(Property.forName("orgId").in(positionEmpDao.findCurrentAndChildOrgIds(SecurityContext.getEmpId())));
            addNewCondition = true;
        } else if (hasOrg) {         // 查询个人所属组织机构
            disjunction.add(Property.forName("orgId").in(positionEmpDao.findEmpOrgIds(SecurityContext.getEmpId())));
            addNewCondition = true;
        }
        if (hasPosition) {          // 获得个人的岗位
            disjunction.add(Property.forName("positionId").in(positionEmpDao.findEmpPositionIds(SecurityContext.getEmpId())));
            addNewCondition = true;
        }
        if (hasParam) {             // 获得个人系统
            disjunction.add(Property.forName("orgId").in(positionEmpDao.findEmpPramOrgIds(SecurityContext.getEmpId())));
            addNewCondition = true;
        }
