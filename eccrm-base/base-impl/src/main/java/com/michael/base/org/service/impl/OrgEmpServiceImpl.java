...
public class OrgEmpServiceImpl implements OrgEmpService {
    ...
    public void deleteByIds(String[] ids) {
        ...
        for (String id : ids) {
            if (StringUtils.isNotEmpty(id)) {
                OrgEmp orgEmp = orgEmpDao.findById(id);
                Assert.notNull(orgEmp, "删除失败!数据不存在!请刷新后重试!");
                String orgId = orgEmp.getOrgId();
                orgService.addEmp(orgId, -1);
            }
        }
    }
    ...
    public Integer addEmp(String orgId, String[] empIds) {
        ...
        orgService.addEmp(orgId, counts);
        return counts;
    }
    ...
    public Integer removeEmp(String orgId, String[] empIds) {
        ...
        return orgService.addEmp(orgId, -counts);
    }
